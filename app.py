...
if completed_exercises.empty:
    ...
    st.subheader("Progress Chart (ORM Over Time)")
    orm_progress = workout_log.loc[workout_log["Exercise"].isin(completed_exercises["Exercise"]), ["Exercise", "Week", "ORM"]]
    
    fig, ax = plt.subplots()
    for exercise in orm_progress["Exercise"].unique():
        ex_data = orm_progress[orm_progress["Exercise"] == exercise]
        ax.plot(ex_data["Week"], ex_data["ORM"], marker="o", label=exercise)
    
    ax.set_title("ORM Progress Over Time")
    ax.set_xlabel("Week")
    ax.set_ylabel("One Rep Max (ORM)")
    ax.legend()
    st.pyplot(fig)
    
    if st.button("Show Completed Days in Week"):
        completed_days = workout_log.loc[(workout_log["Week"] == selected_week) & (workout_log["Muscle Group"].isin(full_muscle_groups))]
