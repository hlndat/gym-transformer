...
def load_workout_log(service, file_name="workout_log.csv"):
...
def save_workout_log(service, df, to_drive=True, user_workout_log="workout_log.csv"):
    df.to_csv(user_workout_log, index=False)
    file_id = get_file_id(service, user_workout_log)
    if to_drive:
        def upload_to_drive():
            media = MediaFileUpload(user_workout_log, mimetype='text/csv')
            if file_id:
                service.files().update(fileId=file_id, media_body=media).execute()
            else:
                file_metadata = {'name': user_workout_log}
...
def login():
    ...
    if st.button("Log In"):
        users = load_users()
        hashed_password = hash_password(password)
        if username in users['username'].values and hashed_password in users['password'].values:
...
def dashboard():
    ...
    user_workout_file = f"{st.session_state['username']}_workout_log.csv"
    if st.button("Reload"):
        workout_log = load_workout_log(service, user_workout_file)
        st.success("Reloaded workout log from Google Drive successfully!")
        st.rerun()
    if not os.path.exists(user_workout_file):
        workout_log = load_workout_log(service, load_workout_log)
        workout_log.to_csv(user_workout_file, index=False)
    else:
        workout_log = pd.read_csv(user_workout_file)
    ...
    if not is_completed:
        ...
        if st.button("Log Exercise", disabled=is_completed):
            ...
            save_workout_log(service, workout_log, user_workout_log=user_workout_file)
